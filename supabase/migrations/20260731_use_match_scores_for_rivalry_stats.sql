-- Match scores are the canonical source for head-to-head results. Elo history
-- can be recalculated and may temporarily retain an older result after edits.
...
AS $$
	...
	raw_matches AS (
		...
		WHERE sm.status = 'completed' AND sm.match_type = 'singles'
	),
...
-- Existing JSON contains results calculated with the previous winner source.
-- Force the first request after deployment to rebuild every player snapshot.
